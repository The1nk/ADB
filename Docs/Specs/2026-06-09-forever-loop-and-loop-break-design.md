...
- **Inside a Parallel branch (unsupported — two cases).**
  - *Terminal Loop-Break (static):* Loop-Break has no output port, so a branch whose only path ends in Loop-Break cannot reach its Join. `ParallelControlFlowExecutor` requires every branch to converge on exactly one Join (`FindConvergentJoin` BFS-walks each branch's outgoing edges *before* running them), so such a graph fails at runtime with the existing "branches must converge on exactly one Join" error — identical to placing `End` terminally in a branch.
  - *Conditionally-reached Loop-Break (runtime):* A branch may have a gate node that routes to Loop-Break on one output port while its other port reaches the Join directly — the Join is still reachable via a sibling edge, so `FindConvergentJoin` succeeds and the graph runs. At runtime the gate takes the Loop-Break path. Because there is no enclosing loop *inside the branch* the break unwinds to the Parallel boundary. `ParallelControlFlowExecutor.RunBranchAsync` detects a branch sub-walk that returns `WalkOutcome.IsBreak`, converts it to `WalkOutcome.Failed("Loop-Break cannot cross a Run Parallel branch boundary; place Loop-Break after the Join.", ...)`, and records the violation. After `Task.WhenAll`, the aggregation checks for a recorded break-violation first and returns `ControlFlowResult.Halt` **unconditionally** — before any `ParallelErrorStrategy` or `someFailed`-wiring logic is reached. A boundary-crossing break is a graph authoring error, not a recoverable branch failure; it cannot be downgraded to a warning or routed through `someFailed` under any strategy.
  - **Supported pattern:** to break a loop based on a parallel outcome, set a variable inside the branch and place Loop-Break *after* the Join (in the sequential body). All three cases — static convergence failure, runtime rejection, and the after-Join success path — are covered by tests (§5).
...
- Loop-Break terminal inside a Parallel branch fails with the existing "must converge on exactly one Join" error (documents the static boundary).
- Loop-Break *conditionally reached* inside a Parallel branch (Join still reachable via sibling edge, so convergence passes) surfaces as a failure at runtime — `ParallelControlFlowExecutor` converts the branch sub-walk's break outcome to `WalkOutcome.Failed("Loop-Break cannot cross a Run Parallel branch boundary; place Loop-Break after the Join.")` rather than silently swallowing it; the run reports failure and the allSucceeded path is never taken. This halt is **unconditional**: it fires regardless of `ParallelErrorStrategy` or whether `someFailed` is wired (a second regression test pins the `Continue`-strategy + unwired-`someFailed` case explicitly).
