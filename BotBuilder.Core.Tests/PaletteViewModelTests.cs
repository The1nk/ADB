...
public class PaletteViewModelTests
{
    ...
    public void Categories_GroupBuiltInsByCategory()
    {
        ...
        var android = palette.Categories.Single(c => c.Name == "Android");
        Assert.Equal(18, android.Items.Count); // Tap, Swipe, Long Press, Press Back, Send Text, Press Key, Enable ADB Keyboard, Restore Keyboard, Launch App, Install APK, Screenshot, Find Image, Wait for Image, Assert Image Absent, Read Text, Find Text, Wait for Text, Assert Text Absent
    ...
    public void ClearingSearch_RestoresAll()
    {
        ...
        Assert.Equal(54, palette.Categories.SelectMany(c => c.Items).Count()); // 11 Control Flow + 4 Data + 1 Scripting + 6 Input + 8 Screen + 18 Android + 5 Browser + 1 Window
    }
