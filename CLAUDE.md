...
│   ├── Picker/                 # Coordinate picker VMs (CoordinatePickerViewModel, RegionSelection, ColorHex)
...
| **Measure Bar** | AdbCore/Actions/BuiltIn/BarMeasureCore.cs | Reads a solid-fill bar's value by scanning the ROI centerline (`screen.measureBar` / `android.measureBar`). Classifies pixels by **Fill** and/or **Empty** hex color (nearest-color when both set; within-`tolerance` of Fill, or NOT-within-`tolerance` of Empty, when one is set; rejects equal fill/empty), maps the leading filled fraction onto `[minValue, maxValue]` (default 0..15) in the chosen `direction`. Writes the integer to `resultVar` (default `bar`) plus `<resultVar>Fraction`. Reads a fresh capture or a `Stored` frame via `FrameSourceResolver`. The properties panel's **Pick fill/empty colour…** buttons (`ColorDropperDialog`, gated on `PropertiesViewModel.SupportsColorPicking`) capture the node's target and let you click a pixel to sample its `#RRGGBB` hex straight into `fillColor`/`emptyColor` (via the zoomable `ZoomPanImageHost` picker — zoom in for single-pixel accuracy); the fields stay hand-editable. The `fillColor`/`emptyColor` fields render as `ConfigFieldType.Color` — a hex textbox with a live colour **swatch** (`HexToBrushConverter` → `ColorHex.TryParse`). |
| **Get Pixel Color** | AdbCore/Actions/BuiltIn/PixelReadCore.cs | Reads one pixel's color into variables (`screen.getPixelColor` / `android.getPixelColor`): `<resultVar>Hex` (`#RRGGBB`), `<resultVar>R/G/B` (default prefix `pixel`) at config point `x`,`y`. The properties panel offers a **Pick…** button (both TypeKeys are registered in `CoordinateFieldMap`) that opens the zoomable coordinate picker to set `x`,`y` by clicking the target. Reads a fresh capture or a `Stored` frame via `FrameSourceResolver`. Read-only — branch on the variables. |
...
| **Frame Pickers (zoom/pan)** | AdbUi.Theme/Controls/ZoomPanImageHost.xaml(.cs) (+ ViewportTransform.cs) | Shared zoomable/pannable image control behind the four "click/drag on a captured frame" tools — `CoordinatePickerDialog`, `RegionPickerDialog`, `ColorDropperDialog` (BotBuilder) and BotCapture's `RegionSelectView`. Mouse wheel zooms toward the cursor, middle-drag pans, with a −/%/+/Fit/100% toolbar and `+`/`−`/`0` keys; the image is sized exactly to `source × scale` (no letterbox), NearestNeighbor when zoomed ≥1× for pixel-accurate picking. Consumers speak only in source-pixel coords (`ImagePointerEventArgs`); overlay dots / preview-rect reproject on zoom. Pure geometry in `ViewportTransform` (fit/clamp/step, zoom-to-cursor, point↔source) is unit-tested. An app-wide themed `ScrollBar` (AdbUi.Theme/Themes/Controls.xaml, using `ScrollBarThumbBrush`) keeps the newly-visible scrollbars on-theme in Dark/HighContrast. |
